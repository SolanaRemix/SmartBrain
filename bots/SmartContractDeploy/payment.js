...
async function verifySubscription(userIdentifier) {
  try {
    ...
    const subscriptions = await stripe.subscriptions.list({
      ...
      status: 'active',
      price: process.env.SMARTCONTRACT_DEPLOY_PRICE_ID,
      limit: 1
    });
...
async function requireActiveSubscription(req, res, next) {
  ...
  if (!isActive) {
    return res.status(402).json({
      ...
      message: 'Active SmartContractDeploy subscription required ($9/month)',
      subscriptionUrl: `${req.protocol}://${req.get('host')}/subscribe/deploy`,
      price: '$9/month'
    });
  }
